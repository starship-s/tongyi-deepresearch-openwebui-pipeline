...
class Tools:
    ...
    async def google_scholar(
        ...
        formats results with the scholar-specific header.
        """
        ...
        if len(prefixed) == 1:
            ...
            result = await self._search_single_query(prefixed[0], scholar=True)
        else:
            await emit(f"Scholar searching {len(prefixed)} queries concurrently\u2026")
            results = await asyncio.gather(
                *(self._search_single_query(q, scholar=True) for q in prefixed)
            )
            result = "\n=======\n".join(results)
